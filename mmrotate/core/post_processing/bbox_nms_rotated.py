...
def multiclass_nms_rotated_F(multi_bboxes,multi_feats,
                           multi_scores,
                           score_thr,
                           nms,
                           max_num=-1,
    ...
    if multi_bboxes.shape[1] > 5:
        ...
        feats = multi_feats[:, None].expand(
            multi_scores.size(0), num_classes, 1024)
    ...
    if return_inds:
        return torch.cat([bboxes, feats, scores[:, None]], 1), labels, keep
    else:
        return torch.cat([bboxes, feats, scores[:, None]], 1), labels
